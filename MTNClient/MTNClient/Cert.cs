...
using MTNClient.PrivKey;

namespace MTNClient
{
    internal class Cert
    {
        public static Cert[] GetCerts(string driveName)
        {
            ...
            {
                if (drive.IsReady && driveName == drive.Name)  // [2020.09.11 JYYun] 추가
                {
                    paths.Add(Path.Combine(drive.Name, "NPKI"));
                }
            }

            // TODO : (로컬테스트용) 오픈시 주석처리. 테스트를 위한 임시 폴더에서 인증서를 찾는다.
            //paths.Add(@"D:\Temp2\Users\" + Environment.UserName + @"\AppData\LocalLow\NPKI"); // [2020.09.11 JYYun] 추가
            //paths.Add(@"D:\Temp2\NPKI"); // [2020.09.11 JYYun] 추가
        ...
        public Cert(string path)
        {
            ...
            // [2020.09.23 JYYun] using 뺀 객체로 생성
            {
                var cert = new X509Certificate2(File.ReadAllBytes(this.m_der[0]));
                this.DerFullFileName = this.m_der[0]; // [2020.09.04 JYYun] 추가
                this.KeyFullFileName = this.m_key[0]; // [2020.09.04 JYYun] 추가
        ...
        public string   DerFullFileName { get; } // [2020.09.04 JYYun] 추가
        public string   KeyFullFileName { get; } // [2020.09.04 JYYun] 추가
